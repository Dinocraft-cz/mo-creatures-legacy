...
public class CommandMoCTP extends CommandBase {
        ...
        if (player != null && ownerPetData != null)
        {
            for (int i = 0; i < ownerPetData.getTamedList().tagCount(); i++)
            {
                ...
                if (nbt.hasKey("PetId") && nbt.getInteger("PetId") == petId)
                {
                    ...
                    if (!teleportLoadedPet(world, player, petId, par1ICommandSender))
                    {
                        ...
                        boolean result = teleportLoadedPet(world, player, petId, par1ICommandSender); // attempt to TP again
                        if (!result)
                        {
    ...
    public boolean teleportLoadedPet(WorldServer world, EntityPlayerMP player, int petId, ICommandSender par1ICommandSender)
    {
        for (int j = 0; j < world.loadedEntityList.size(); j++)
        {
            Entity entity = (Entity) world.loadedEntityList.get(j);
            // search for entities that are MoCEntityAnimal's
            if (IMoCTameable.class.isAssignableFrom(entity.getClass()) && !((IMoCTameable)entity).getName().isEmpty() && ((IMoCTameable)entity).getOwnerPetId() == petId)
            {
